...
#define AR8XXX_ARL_READ_WORK_DELAY	6000
...
static struct proc_dir_entry *proc_switch_dir;
static struct proc_dir_entry *proc_arl_table;
...
ar8xxx_phy_init(struct ar8xxx_priv *priv)
{
	...
	for (i = 0; i < AR8XXX_NUM_PHYS; i++) {
		...
		if (priv->phy_settings[i].link_disable) {
			mdiobus_write(bus, i, MII_BMCR, BMCR_PDOWN);
		} else {
			regval = 0;
			regval |= ADVERTISE_PAUSE_CAP | ADVERTISE_PAUSE_ASYM;

			if (priv->phy_settings[i].advertise_speed >= 100 && priv->phy_settings[i].advertise_fdx)
				regval |= ADVERTISE_100FULL;

			if (priv->phy_settings[i].advertise_speed >= 100 && priv->phy_settings[i].advertise_hdx)
				regval |= ADVERTISE_100HALF;

			if (priv->phy_settings[i].advertise_speed >= 10 && priv->phy_settings[i].advertise_fdx)
				regval |= ADVERTISE_10FULL;

			if (priv->phy_settings[i].advertise_speed >= 10 && priv->phy_settings[i].advertise_hdx)
				regval |= ADVERTISE_10HALF;

			mdiobus_write(bus, i, MII_ADVERTISE, regval);

			if (priv->phy_settings[i].advertise_speed == 1000 && priv->phy_settings[i].advertise_fdx && ar8xxx_has_gige(priv))
...
ar8xxx_sw_hw_apply(struct switch_dev *dev)
{
	...
	/* re-init ports to prevent disconnected downstream clients (when changing port vlan) */
	ar8xxx_phy_init(priv);
...
ar8xxx_sw_reset_switch(struct switch_dev *dev)
{
	...
	for (i = 0; i < AR8XXX_NUM_PHYS; i++) {
		priv->phy_settings[i].link_disable = false;
		priv->phy_settings[i].advertise_fdx = true;
		priv->phy_settings[i].advertise_hdx = true;
		if(ar8xxx_has_gige(priv))
			priv->phy_settings[i].advertise_speed = 1000;
		else
			priv->phy_settings[i].advertise_speed = 100;
	}
	...
	priv->arl_cache_age_time = AR8XXX_DEFAULT_ARL_AGE_TIME;
...
int
ar8xxx_sw_get_arl_cache_age_time(struct switch_dev *dev, const struct switch_attr *attr,
                   struct switch_val *val)
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);
	val->value.i = priv->arl_cache_age_time;
	return 0;
}

int
ar8xxx_sw_set_arl_cache_age_time(struct switch_dev *dev, const struct switch_attr *attr,
			   struct switch_val *val)
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);
	int age_time = val->value.i;
	...
	priv->arl_cache_age_time = age_time;
	return 0;
}
			...
			struct switch_val *val)
{
	...
	for (j = 0; j < priv->dev.ports; ++j) {
		for (k = 0; k < AR8XXX_ARL_LIST_HASHSIZE; k++) {
			list_for_each_safe(ptr, n, &priv->arl_table[k].list) {
				a = list_entry(ptr, struct arl_entry, list);

				if (a->port != j)
					continue;

				// Prevent bug where entries get stuck for an extremely long time if jiffies rolls over when unsigned long = 4 bytes
				if (a->last_seen > jiffies)
					a->last_seen = jiffies;

				if (((jiffies - a->last_seen) / HZ) > priv->arl_cache_age_time) {
					list_del(ptr);
					kfree(a);
				} else if (count < AR8XXX_MAX_ARL_RETURNED) {
					len += snprintf(buf + len, sizeof(priv->arl_buf) - len,
							"%d\t%02x:%02x:%02x:%02x:%02x:%02x\t%lu\n",
							a->port,
							a->mac[5], a->mac[4], a->mac[3],
							a->mac[2], a->mac[1], a->mac[0], (jiffies - a->last_seen) / HZ);
					count++;
				} else if (count == AR8XXX_MAX_ARL_RETURNED) {
					len += snprintf(buf + len, sizeof(priv->arl_buf) - len,
...
static struct arl_seq_iter {
	struct ar8xxx_priv *priv;
	struct list_head *current_arl_ptr;
	int port_idx;
	int arl_table_idx;
};

#define list_for_each_safe_continue(pos, n, head) \
	for ((pos) = ((pos) == NULL) ? (head)->next : (pos)->next, n = (pos)->next; (pos) != (head); \
	(pos) = n, n = (pos)->next)

// re-entrant version of ar8xxx_sw_get_arl_table
static struct arl_entry *
get_next_arl_entry(struct arl_seq_iter *iter) {
	struct ar8xxx_priv *priv = iter->priv;
	struct list_head *n;
	struct arl_entry *a;

	for (; iter->port_idx < priv->dev.ports; iter->port_idx++, iter->arl_table_idx = 0) {
		for (; iter->arl_table_idx < AR8XXX_ARL_LIST_HASHSIZE; iter->arl_table_idx++, iter->current_arl_ptr = NULL) {
			list_for_each_safe_continue(iter->current_arl_ptr, n, &priv->arl_table[iter->arl_table_idx].list) {
				a = list_entry(iter->current_arl_ptr, struct arl_entry, list);

				if (a->port != iter->port_idx)
					continue;
...
static void *
proc_arl_table_seq_start(struct seq_file *s, loff_t *pos)
{
...
static void *
proc_arl_table_seq_next(struct seq_file *s, void *v, loff_t *pos)
{
	return get_next_arl_entry(s->private);
}

static void
proc_arl_table_seq_stop(struct seq_file *s, void *v)
{
	struct arl_seq_iter *iter = s->private;
	struct ar8xxx_priv *priv = iter->priv;
	mutex_unlock(&priv->reg_mutex);
...
static int
proc_arl_table_seq_show(struct seq_file *s, void *v)
{
	struct arl_entry *a = v;
	seq_printf(s, "%d %d %02x:%02x:%02x:%02x:%02x:%02x %lu %lu\n",
		  a->port,
		  a->vid,
		  a->mac[5], a->mac[4], a->mac[3],
		  a->mac[2], a->mac[1], a->mac[0],
		  (jiffies + (HZ / 2) - a->first_add) / HZ,
		  (jiffies - a->last_seen) / HZ);
...
static int
proc_arl_table_open(struct inode *inode, struct file *file)
{
	// return single_open(file, proc_arl_table_show, PDE(inode)->data);
	int ret = seq_open(file, &proc_arl_table_seq_ops);
	if (!ret) {
		struct arl_seq_iter *iter = kzalloc(sizeof(struct arl_seq_iter), GFP_KERNEL);
		iter->priv = PDE(inode)->data;
...
static int
proc_arl_table_release(struct inode *inode, struct file *file)
{
...
int
ar8xxx_sw_get_port_disable(struct switch_dev *dev,
				      const struct switch_attr *attr,
				      struct switch_val *val)
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);

	if (val->port_vlan >= AR8XXX_NUM_PHYS+1 || val->port_vlan == 0)
		return -EINVAL;

	val->value.i = priv->phy_settings[val->port_vlan-1].link_disable ? 1 : 0;

	return 0;
}

int
ar8xxx_sw_set_port_disable(struct switch_dev *dev,
				      const struct switch_attr *attr,
				      struct switch_val *val)
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);
	...
	priv->phy_settings[val->port_vlan-1].link_disable = val->value.i ? 1 : 0;

	return 0;
}

int
ar8xxx_sw_get_fdx_advertise(struct switch_dev *dev,
			    const struct switch_attr *attr,
			    struct switch_val *val)
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);
	...
	val->value.i = priv->phy_settings[val->port_vlan-1].advertise_fdx;

	return 0;
}

int
ar8xxx_sw_set_fdx_advertise(struct switch_dev *dev,
			    const struct switch_attr *attr,
			    struct switch_val *val)
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);
	...
	if (!val->value.i && priv->phy_settings[val->port_vlan-1].advertise_speed == 1000)
		return -EINVAL;

	priv->phy_settings[val->port_vlan-1].advertise_fdx = val->value.i ? 1 : 0;

	return 0;
}

int
ar8xxx_sw_get_hdx_advertise(struct switch_dev *dev,
			    const struct switch_attr *attr,
			    struct switch_val *val)
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);
	...
	val->value.i = priv->phy_settings[val->port_vlan-1].advertise_hdx;

	return 0;
}

int
ar8xxx_sw_set_hdx_advertise(struct switch_dev *dev,
			    const struct switch_attr *attr,
			    struct switch_val *val)
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);
	...
	priv->phy_settings[val->port_vlan-1].advertise_hdx = val->value.i ? 1 : 0;

	return 0;
}

int
ar8xxx_sw_get_speed_advertise(struct switch_dev *dev,
			      const struct switch_attr *attr,
			      struct switch_val *val)
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);
	...
	val->value.i = priv->phy_settings[val->port_vlan-1].advertise_speed;

	return 0;
}

int
ar8xxx_sw_set_speed_advertise(struct switch_dev *dev,
			      const struct switch_attr *attr,
			      struct switch_val *val)
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);
	...
	switch(val->value.i) {
		case 1000:
			if (ar8xxx_has_gige(priv) && priv->phy_settings[val->port_vlan-1].advertise_fdx)
				priv->phy_settings[val->port_vlan-1].advertise_speed = 1000;
			else
				return -EINVAL;
			break;
		case 100:
			priv->phy_settings[val->port_vlan-1].advertise_speed = 100;
			break;
		case 10:
			priv->phy_settings[val->port_vlan-1].advertise_speed = 10;
			break;
		default:
			return -EINVAL;
	}
...
int
ar8xxx_sw_get_port_stp_state(struct switch_dev *dev,
			     const struct switch_attr *attr,
			     struct switch_val *val)
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);
	...
	val->value.i = priv->port_stp_state[port];

	return 0;
}

int
ar8xxx_sw_set_port_stp_state(struct switch_dev *dev,
			     const struct switch_attr *attr,
			     struct switch_val *val)
{
	struct ar8xxx_priv *priv = swdev_to_ar8xxx(dev);
	...
	if (val->value.i == AR8216_PORT_STATE_DISABLED || val->value.i == AR8216_PORT_STATE_FORWARD) {
		priv->port_stp_state[port] = val->value.i;
		return 0;
	}
...
static void
ar8xxx_arl_read_work_func(struct work_struct *work) {
	struct ar8xxx_priv *priv;
	struct mii_bus *bus;
	const struct ar8xxx_chip *chip;
	int m, n = 0;
	struct arl_read_tmp_entry a;
	bool found_existing_entry = false;
	u32 status;

	priv = container_of(work, struct ar8xxx_priv, arl_read_work.work);
	bus = priv->mii_bus;
	chip = priv->chip;
	...
	// merge temp table into cache
	for (m = 0; m < AR8XXX_NUM_ARL_RECORDS; m++) {
		struct list_head *ptr;
		struct arl_entry *tmp_entry;
		...
		n = AR8XXX_ARL_LIST_HASH(a.mac);

		// Step 1: check to see if MAC address is in cache.  If so, update port and last_seen.
		list_for_each(ptr, &(priv->arl_table[n].list)) {
			tmp_entry = list_entry(ptr, struct arl_entry, list);

			if (!memcmp(tmp_entry->mac, a.mac, sizeof(a.mac))) {
				tmp_entry->port = a.port;
				tmp_entry->vid = a.vid;
				tmp_entry->last_seen = jiffies;
		...
		if (!found_existing_entry) {
			tmp_entry = (struct arl_entry *)kzalloc(sizeof(struct arl_entry), GFP_KERNEL);

			memcpy(tmp_entry->mac, a.mac, sizeof(a.mac));
			tmp_entry->last_seen = jiffies;
			tmp_entry->port = a.port;
			tmp_entry->vid = a.vid;
			tmp_entry->first_add = jiffies;
			list_add(&tmp_entry->list, &priv->arl_table[n].list);
		}
	...
	schedule_delayed_work(&priv->arl_read_work,
			      msecs_to_jiffies(AR8XXX_ARL_READ_WORK_DELAY));
...
ar8xxx_create(void)
{
	...
	INIT_DELAYED_WORK(&priv->arl_read_work, ar8xxx_arl_read_work_func);
...
ar8xxx_free(struct ar8xxx_priv *priv)
{
	...
	struct arl_entry *tmp;
	unsigned long i;

	if (priv->chip && priv->chip->cleanup)
		priv->chip->cleanup(priv);

	for (i = 0; i < AR8XXX_ARL_LIST_HASHSIZE; i++) {
		if (priv->arl_table[i].list.next) {
			list_for_each_safe(c, n, &priv->arl_table[i].list) {
				tmp = list_entry(c, struct arl_entry, list);
				list_del(c);
				kfree(tmp);
			}
		}
...
ar8xxx_start(struct ar8xxx_priv *priv)
{
	...
	for (i = 0; i < AR8XXX_ARL_LIST_HASHSIZE; i++)
		INIT_LIST_HEAD(&priv->arl_table[i].list);
...
ar8xxx_phy_probe(struct phy_device *phydev)
{
	...
	proc_switch_dir = proc_mkdir(swdev->devname, NULL);
	if (proc_switch_dir) {
		proc_arl_table = proc_create_data("arl", 0444, proc_switch_dir, &proc_arl_table_fops, priv);
		if (!proc_arl_table) {
			remove_proc_entry(proc_switch_dir->name, NULL);
...
ar8xxx_phy_remove(struct phy_device *phydev)
{
	...
	cancel_delayed_work_sync(&priv->arl_read_work);
	ar8xxx_free(priv);
	remove_proc_entry(proc_arl_table->name, proc_switch_dir);
	remove_proc_entry(proc_switch_dir->name, NULL);
}
