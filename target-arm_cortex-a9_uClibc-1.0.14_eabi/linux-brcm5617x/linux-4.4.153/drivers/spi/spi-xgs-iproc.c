...
spinlock_t  *qspi_hw_lock;
EXPORT_SYMBOL(qspi_hw_lock);
int    *qspi_bus_state;
EXPORT_SYMBOL(qspi_bus_state);
int    *bspi_enabled;
EXPORT_SYMBOL(bspi_enabled);
...
int iproc_qspi_flash_read(u32 addr, void *buffer, int len)
{
    u8 en4b[] = { OPCODE_EN4B };
    u8 ex4b[] = { OPCODE_EX4B };
    u8 cmd[] = { OPCODE_READ, 0x0, 0x0, 0x0, 0x0 };
    struct bcmspi_priv *priv = spi_master_get_devdata(default_master);
    ...
    DBG("%s: read %#010x, len %d\n", __func__, addr, len);
    DBG("%#x = %02x %02x %02x %02x\n", addr, cmd[1], cmd[2], cmd[3], cmd[4]);

    /* always use 4 bytes */
    ret = bcmspi_simple_transaction(xp, en4b, 1, NULL, 0);
    if (ret) return ret;

    ret = bcmspi_simple_transaction(xp, cmd, sizeof(cmd), buffer, len);
    if (ret) return ret;

    ret = bcmspi_simple_transaction(xp, ex4b, 1, NULL, 0);
    return ret;
}
EXPORT_SYMBOL(iproc_qspi_flash_read);
...
static void bcmspi_hw_init(struct bcmspi_priv *priv)
{
    ...
    priv->bspi_enabled = 1;
    bspi_enabled = &priv->bspi_enabled;
    bcmspi_disable_bspi(priv);
}
