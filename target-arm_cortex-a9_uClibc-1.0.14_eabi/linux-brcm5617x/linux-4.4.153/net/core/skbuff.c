...
int (*ubnt_memleak_hook)(const char *ops, void *addr, int size);
EXPORT_SYMBOL(ubnt_memleak_hook);
int ubnt_memleak_handler(const char *ops, void *addr, int size) {
    if (unlikely(ubnt_memleak_hook))
        return ubnt_memleak_hook(ops, addr, size);
    return 0;
}

#include "skbuff_recycle.c"
...
out:
	if (skb)
		ubnt_memleak_handler("alloc", skb, size);
	return skb;
nodata:
