...
#include <inttypes.h>
#include "blobmsg.h"
#include "blobmsg_jansson.h"

#include <jansson.h>

bool blobmsg_add_object(struct blob_buf *b, json_t *obj)
{
	const char *key;
	json_t *value;

	json_object_foreach(obj, key, value) {
		if (!blobmsg_add_json_element(b, key, value))
			return false;
	}
	return true;
}

static bool blobmsg_add_array(struct blob_buf *b, json_t *a)
{
	int i;
	json_t * value;

	json_array_foreach(a, i, value) {
		if (!blobmsg_add_json_element(b, NULL, value))
			return false;
	}
...
bool blobmsg_add_json_element(struct blob_buf *b, const char *name, json_t *obj)
{
	bool ret = true;
	void *c;
	...
	case JSON_ARRAY:
		...
		ret = blobmsg_add_array(b, obj);
		blobmsg_close_array(b, c);
		break;
	case JSON_STRING:
		blobmsg_add_string(b, name, json_string_value(obj));
		break;
	case JSON_TRUE:
	...
	case JSON_FALSE:
		...
		break;
	case JSON_INTEGER:
		blobmsg_add_u32(b, name, json_integer_value(obj));
		break;
	case JSON_REAL:
		blobmsg_add_double(b, name, json_real_value(obj));
		break;
	case JSON_NULL:
...
static bool __blobmsg_add_json(struct blob_buf *b, json_t *obj)
{
	bool ret = false;
...
bool blobmsg_add_json_from_file(struct blob_buf *b, const char *file)
{
	json_t *obj;
	json_error_t jerr;
	obj = json_load_file(file, 0, &jerr);
	if (!obj) {
		return false;
	}
	return __blobmsg_add_json(b, obj);
}
