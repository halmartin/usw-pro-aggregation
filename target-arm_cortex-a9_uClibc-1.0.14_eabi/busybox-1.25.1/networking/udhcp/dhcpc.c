		...
		*curr = xmalloc_optname_optval(temp, &dhcp_optflags[i], opt_name);
		...
		if (code == DHCP_SUBNET && temp[-OPT_DATA + OPT_LEN] == 4) {
			...
			*curr = xasprintf("mask=%u", mton(subnet));
			putenv(*curr++);
		} else if (code == DHCP_VENDOR_SPEC) {
			char url[256]; /* option allows a max of 255 bytes */
			uint8_t ucode;
			uint8_t ulen;
			uint8_t uvalid = 1;
			uint8_t *upos = temp;
			...
			uint8_t* p;
			int remaining;
			static const char * valid_url_char = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~:/?#[]@!$&'()*+,;=`.";
			while (uvalid) {
				uvalid = 0;
				ucode = upos[0];
				ulen = upos[1];
				switch (ucode) {
					case 1:
						if (ulen == 4) {
							/* code 1: unifi_addr */
							*curr = xasprintf("unifi_addr=%u.%u.%u.%u",upos[2],upos[3],upos[4],upos[5]);
							putenv(*curr++);
							uvalid = 1;
						}
						break;
					case 2:
						if (ulen > 0) {
							/* code 2: unifi_controller URL */
							uvalid = 1;
							for (p=upos+2,dp=(uint8_t*)url,remaining=ulen; remaining; p++,dp++,remaining--) {
								/* while it's somewhat spec variable, permit UTF-8 encoded characters */
								if ((*p > 0x7f) || (strchr(valid_url_char, *p) != NULL)) {
									*dp = *p;
								} else {
									log1("unifi_controller URL contains invalid characters, skipping");
									uvalid = 0;
									break;
								}
							}
							if (uvalid) {
								url[ulen] = '\0';
								*curr = xasprintf("unifi_controller=%s", url);
								putenv(*curr++);
							}
						}
						break;
